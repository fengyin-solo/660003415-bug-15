...
export function buildGraph() {
  ...
  COGNATE_SETS.forEach((cs, ci) => {
    ...
    nodes.push({ id: rootId, rootId, word: cs.root, language: 'Proto-IE', meaning: cs.meaning, family: cs.family, era: '公元前5000年' })
    Object.entries(cs.languages).forEach(([lang, word]) => {
      if (!word || word === '-') return
      const nid = ci + '_' + lang
      nodes.push({ id: nid, rootId, word, language: lang, meaning: cs.meaning, family: cs.family, era: '现代' })
